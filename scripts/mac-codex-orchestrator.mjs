...
export function buildRemoteScript(options) {
  ...
  const supervisedBridgeCommands = options.supervisedGui
    ? [
        ...
        '  local status="$1" exit_code="$2" marker="$3" failure="$4" observed_after="$5"',
        ...
        `  print -r -- '{"version":1,"nonce":"'$SUPERVISED_TOKEN'","expectedHead":"'$EXPECTED_HEAD'","observedHeadBefore":"'$EXPECTED_HEAD'","observedHeadAfter":'"$observed_after_json"',"commandId":"${SUPERVISED_COMMAND_ID}","commandSha256":"${SUPERVISED_COMMAND_SHA256}","status":"'"$status"'","exitCode":'"$exit_code"',"markerConfirmed":'"$marker"',"failureClass":"'"$failure"'"}' >| "$temporary_attestation"`,
