...
export function buildRemoteScript(options) {
  ...
  const supervisedBridgeCommands = options.supervisedGui
    ? [
        ...
        'fi',
        'if (( BRIDGE_SETUP_OK == 1 )) && { [[ ! -x "$SUPERVISED_HELPER_DIR/mac-2fa-popup-read" ]] ||',
        '   [[ ! -x "$SUPERVISED_HELPER_DIR/mac-2fa-click-allow" ]] ||',
        '   [[ ! -x "$SUPERVISED_HELPER_DIR/mac-settings-2fa-code" ]] ||',
        '   [[ ! -x "$SUPERVISED_HELPER_DIR/mac-2fa-popup-ocr" ]]; }; then',
        '  BRIDGE_SETUP_OK=0',
