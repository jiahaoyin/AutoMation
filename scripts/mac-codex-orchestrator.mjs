...
export function buildRemoteScript(options) {
  ...
  const supervisedBridgeCommands = options.supervisedGui
    ? [
        ...
        'if [[ -e "$SUPERVISED_HELPER_DIR" && { ! -d "$SUPERVISED_HELPER_DIR" || -L "$SUPERVISED_HELPER_DIR"; } ]]; then',
        '  BRIDGE_SETUP_OK=0',
        'fi',
        'if (( BRIDGE_SETUP_OK == 1 )); then /bin/mkdir -p "$SUPERVISED_HELPER_DIR"; fi',
        '/bin/chmod 700 "$SUPERVISED_CONTROL_DIR" "$SUPERVISED_PRODUCTION_DIR"',
        'if (( BRIDGE_SETUP_OK == 1 )); then /bin/chmod 700 "$SUPERVISED_HELPER_DIR"; fi',
        ...
        '}',
        'if (( BRIDGE_SETUP_OK == 1 )) && { ! compile_supervised_helper "mac-2fa-popup-read" -framework ApplicationServices -framework AppKit ||',
