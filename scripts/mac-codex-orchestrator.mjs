...
export function buildRemoteScript(options) {
  ...
  const supervisedBridgeCommands = options.supervisedGui
    ? [
        ...
        'if (( BRIDGE_SETUP_OK == 1 )) && { ! /usr/bin/xcrun swiftc -module-cache-path "$SWIFT_MODULE_CACHE" -O -o "$SUPERVISED_HELPER_DIR/mac-2fa-popup-read" scripts/swift/mac-2fa-popup-read.swift -framework ApplicationServices -framework AppKit >/dev/null 2>&1 ||',
        ...
        '  /usr/bin/xcrun swiftc -module-cache-path "$SWIFT_MODULE_CACHE" -O -o "$SUPERVISED_HELPER_DIR/.mac-settings-2fa-code.tmp.$$" scripts/swift/mac-settings-2fa-code.swift -framework ApplicationServices -framework AppKit >/dev/null 2>&1 && /bin/mv -f "$SUPERVISED_HELPER_DIR/.mac-settings-2fa-code.tmp.$$" "$SUPERVISED_HELPER_DIR/mac-settings-2fa-code" || true',
        '  /usr/bin/xcrun swiftc -module-cache-path "$SWIFT_MODULE_CACHE" -O -o "$SUPERVISED_HELPER_DIR/.mac-2fa-popup-ocr.tmp.$$" scripts/swift/mac-2fa-popup-ocr.swift -framework ApplicationServices -framework AppKit -framework Vision -framework CoreGraphics -framework ScreenCaptureKit >/dev/null 2>&1 && /bin/mv -f "$SUPERVISED_HELPER_DIR/.mac-2fa-popup-ocr.tmp.$$" "$SUPERVISED_HELPER_DIR/mac-2fa-popup-ocr" || true',
        'fi',
