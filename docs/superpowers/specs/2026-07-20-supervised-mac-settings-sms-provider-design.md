...
The module is enabled only in a macOS supervised GUI session. It reads `APPLE_AUTOMATION_SMS_PHONE` and `APPLE_AUTOMATION_SMS_API_URL` only when both are present. If neither is present, it interactively collects the number and full HTTPS URL from the supervised terminal; interactive collection is preferred. A partial environment configuration is invalid and stops before any UI action. Values are process-local secrets: never persist them, pass them as child arguments, copy them into child environments, log them, or include them in reports/screenshots/errors.
